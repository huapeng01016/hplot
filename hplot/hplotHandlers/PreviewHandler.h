...
#include <afxext.h>
#include "..\hplot\hplotDoc.h"
#include "..\hplot\hplotView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(ChplotView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(ChplotDoc, (CObject*) m_pDocument->GetContainer());
